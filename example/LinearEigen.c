...
//------------------
  ...
  fesp3D_H = BUILDFEMSPACE3D(mesh_H,C_T_P1_3D,BoundCond);
  ...
  n_test_MultiIndexA = 4;
  n_test_MultiIndexB = 1;
  n_anasatz_MultiIndexA = 4;
  n_anasatz_MultiIndexB = 1;
  MultiIndex3D test_MultiIndexA[4] =  {D000,D100,D010,D001};
  MultiIndex3D test_MultiIndexB[1] =  {D000};
  MultiIndex3D anasatz_MultiIndexA[4]={D000,D100,D010,D001};
  ...
  int nev =1;//求解的特征值个数
  double *eigenvalues;
  eigenvalues = malloc(sizeof(double)*(Num_Outloop+1));
  memset(eigenvalues,0.0,(Num_Outloop+1)*sizeof(double)); 
  //InitialPointer(eigenvalues,Num_Outloop+1,0.0);
  double **eigenvectors;
  eigenvectors = malloc((Num_Outloop+1)*sizeof(double*)); 
  eigenvectors[0] = malloc(dof_all*sizeof(double));   
  memset(eigenvectors[0],0.0,dof_all*sizeof(double)); 
  ...
  DNEigenSolver(stiff_matrix,mass_matrix,nev,-1,1.0,eigenvalues,eigenvectors+0,1);

 //第二步，开始多水平校正迭代--------------------------------------------------------------------------------------------------------------------
  double *val, *solution,*rhs;
  double tol,tt1,tt2,tt3,tt4,tt5;
  ...
  for(outloop=0;outloop<Num_Outloop;++outloop)
  {
   ...
   tt2 = GetTime(); 
    ...
    //{
      ...
      CG(stiff_matrix,rhs,solution,1e-5,1000);
  ...
  WriteMatrix(mass_matrix,"../dat/mass_matrix-consistent.dat");
  WriteMatrix(stiff_matrix,"../dat/stiff_matrix-consistent.dat");
  ...
  printf ( "the final eigenvalue=%f, %f\n",eigenvalues[i],eigenvalues[i]-eigenvalues[Num_Outloop]);
  //特征函数估计
  Fespace3D *fesp3D_new; 
  fesp3D_new = BUILDFEMSPACE3D(multilevel->mesh,C_T_P1_3D,BoundCond);
		   ...
		   double *UserFunction,int N_FeConst,double *FeConst)
{
    ...
    return  (1+pow(Quad_x-0.5,2))*Test_Value[1]*Anasatz_Value[1]        +(Quad_x-0.5)*(Quad_y-0.5)*Test_Value[2]*Anasatz_Value[1]  +(Quad_x-0.5)*(Quad_z-0.5)*Test_Value[3]*Anasatz_Value[1]
           +(Quad_x-0.5)*(Quad_y-0.5)*Test_Value[1]*Anasatz_Value[2]    +(1+pow(Quad_y-0.5,2))*Test_Value[2]*Anasatz_Value[2]      +(Quad_y-0.5)*(Quad_z-0.5)*Test_Value[2]*Anasatz_Value[3]
           +(Quad_x-0.5)*(Quad_z-0.5)*Test_Value[3]*Anasatz_Value[1]    +(Quad_y-0.5)*(Quad_z-0.5)*Test_Value[2]*Anasatz_Value[3]  +(1+pow(Quad_z-0.5,2))*Test_Value[3]*Anasatz_Value[3]
           +exp((Quad_x-0.5)*(Quad_y-0.5)*(Quad_z-0.5))*Test_Value[0]*Anasatz_Value[0];
...
double MassMatrix(double Quad_x,double Quad_y,double Quad_z,int N_Test,double *Test_Value,int N_Anasatz,
		   ...
		   double *UserFunction,int N_FeConst,double *FeConst)
{
  return  Test_Value[0]*Anasatz_Value[0]*(1+(Quad_x-0.5)*(Quad_y-0.5)*(Quad_z-0.5));
}
...
{ 
   double tmp;
   tmp=Test_Value[0]*FeConst[0]*AuxFeFun[0]*(1+(Quad_x-0.5)*(Quad_y-0.5)*(Quad_z-0.5));
   //tmp=Test_Value[0]*2*gamma*AuxFeFun[0]*AuxFeFun[0]*AuxFeFun[0];这样有问题
   return tmp;
}
//真实解 
void ExactU(double x,double y,double z, int n, double *values)
