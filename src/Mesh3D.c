...
//按vtk格式输出网格信息
void WriteMeshUM(MESH *mesh, Fefunction3D *fefunction, char *file)
{
    FILE *fp = fopen( file,"w");
    int i,j;
    /* 输出体的信息 */
    fprintf (fp, "# vtk DataFile Version 4.0\n" );
    ...
    /* 输出面的信息 */
    fprintf (fp,  "CELLS %d %d\n", mesh->Num_Volus_Global, mesh->Num_Volus_Global*5 );
    for(i=0;i<mesh->Num_Volus_Global;i++)
    {
     fprintf (fp,  "%d %d %d %d %d\n",4,mesh->Volus[i]->Verts[0],mesh->Volus[i]->Verts[1],mesh->Volus[i]->Verts[2],mesh->Volus[i]->Verts[3] );
    }
...
void GetFacesVolusOwned(MESH *mesh)
{
  ...
  for(i=0;i<mesh->Num_Faces_Global;++i)
  {
    f=mesh->Faces[i]; 
    f->Num_Volus_Owned=2;
    f->Volus_Owned = malloc(sizeof(int)*2);
    f->Volus_Owned[0] = -1;
    f->Volus_Owned[1] = -1;
  }
  VOLU *v; 
   ...
   for(i=0;i<Num_Faces_Global;++i)
   {
    ...
    else
    {
     mesh->Faces[i]->Volus_Owned=
             realloc(mesh->Faces[i]->Volus_Owned,(mesh->Faces[i]->Num_Volus_Owned)* sizeof(int));  
    }
...
*/
//计算误差估计子, 非线性问题 -\laplace u +u^3/2 = f;

void ComputeErrorIndicator(Fespace3D *fespace,  double *fefunction, double *eta, double *total_error)
{
    Fefunction3D *fefunct;
    fefunct = BuildFefunction3D(fespace, fefunction);
    MESH *mesh = fespace->Mesh;
    ELEMENT3D *elem3D;
    elem3D = malloc(1*sizeof(ELEMENT3D));
    InitialElem3D(mesh,elem3D); 
    double Sum_K,Sum_eta, sum_tmp,tmp;
    Sum_eta = 0.0;
    memset(eta, 0.0, mesh->Num_Volus_Global*sizeof(double));
    int i,j; 
    // 记录sol_fef的离散梯度 
    double *gu_h_x = malloc(mesh->Num_Volus_Global*sizeof(double));
    double *gu_h_y = malloc(mesh->Num_Volus_Global*sizeof(double));
    double *gu_h_z = malloc(mesh->Num_Volus_Global*sizeof(double));
    int idx, idx_n;
    Quadrature3D *Quad3D;
    Quad3D = BuildQuad2Mesh3D(mesh, 4);
    double Quad_xi, Quad_eta,Quad_zeta, Quad_W;
    int curr_volu; 
    int N_MultiIndex=4;
    MultiIndex3D MultiIndex1[3]={D100,D010,D001};
    MultiIndex3D MultiIndex2[4]={D000,D200,D020,D002};
    double *Values_FE1,*Values_FE2;
    Values_FE1=malloc(sizeof(double)*3);
    Values_FE2=malloc(sizeof(double)*4);
    double h,norm;   
    FACEE *face;
    double x1,y1,z1,x2,y2,z2;
    double nu[3]; double jump[3];
    double Quad_x,Quad_y,Quad_z;
    //计算得到每个单元上的梯度值
    for(curr_volu=0;curr_volu<mesh->Num_Volus_Global;++curr_volu)
    {
      GetElement3D(mesh,curr_volu,elem3D);
      for(i=0;i<1;i++)//p1元的导数是常数，故取一个点就行
       {
        ...
        GetFefunctValues(fefunct,elem3D,Quad_x,Quad_y,Quad_z,
		       Quad_xi,Quad_eta,Quad_zeta,3,MultiIndex1,Values_FE1);
       }
      gu_h_x[curr_volu]=Values_FE1[0];
      gu_h_y[curr_volu]=Values_FE1[1];
      gu_h_z[curr_volu]=Values_FE1[2];
    }
    ...
    {
      GetElement3D(mesh,curr_volu,elem3D);
      h=sqrt( (elem3D->Vert_X[0]-elem3D->Vert_X[1])*(elem3D->Vert_X[0]-elem3D->Vert_X[1])
             +(elem3D->Vert_Y[0]-elem3D->Vert_Y[1])*(elem3D->Vert_Y[0]-elem3D->Vert_Y[1])
             +(elem3D->Vert_Z[0]-elem3D->Vert_Z[1])*(elem3D->Vert_Z[0]-elem3D->Vert_Z[1]));
      Sum_K = 0.0;
      for(i=0;i<4;i++)
      {
       Quad_xi = Quad3D->Quad_X[i];
       ...
       GetElementCoord(elem3D,Quad_xi,Quad_eta, Quad_zeta,&Quad_x,&Quad_y,&Quad_z);
       GetFefunctValues(fefunct,elem3D,Quad_x,Quad_y,Quad_z,
		       Quad_xi,Quad_eta,Quad_zeta,4,MultiIndex2,Values_FE2);
       sum_tmp = 0.0;
       tmp = -Values_FE2[1]-Values_FE2[2]-Values_FE2[3]+Values_FE2[0]*sqrt(Values_FE2[0])-1;
       sum_tmp = fabs(tmp)*fabs(tmp);
       Sum_K += sum_tmp*Quad_W;
      }//end for i
      eta[curr_volu] += Sum_K*elem3D->Volu*h*h;
      // 遍历单元上的每个面 
      for (i = 0; i <4; ++i)
      {
       face = mesh->Faces[mesh->Volus[elem3D->ID_Num]->Faces[i]];
       x1 = mesh->Verts[face->Verts[1]]->Coord[0]-mesh->Verts[face->Verts[0]]->Coord[0];
       y1 = mesh->Verts[face->Verts[1]]->Coord[1]-mesh->Verts[face->Verts[0]]->Coord[1];
       z1 = mesh->Verts[face->Verts[1]]->Coord[2]-mesh->Verts[face->Verts[0]]->Coord[2]; 
       x2 = mesh->Verts[face->Verts[2]]->Coord[0]-mesh->Verts[face->Verts[0]]->Coord[0];
       y2 = mesh->Verts[face->Verts[2]]->Coord[1]-mesh->Verts[face->Verts[0]]->Coord[1];
       z2 = mesh->Verts[face->Verts[2]]->Coord[2]-mesh->Verts[face->Verts[0]]->Coord[2];   

       nu[0] = z2*y1-z1*y2; nu[1] = z1*x2-z2*x1; nu[2] = x1*y2-x2*y1;
       norm = sqrt(nu[0]*nu[0]+nu[1]*nu[1]+nu[2]*nu[2]);
       nu[0]=nu[0]/norm; nu[1]=nu[1]/norm; nu[2]=nu[2]/norm;
       jump[0] = gu_h_x[curr_volu]; jump[1] = gu_h_y[curr_volu];
       jump[2] = gu_h_z[curr_volu];
       if (face->ID_Boundary==0)
       { // 若不是边界边 
        if (face->Volus_Owned[0]!=curr_volu) idx_n = face->Volus_Owned[0];
        else idx_n = face->Volus_Owned[1];
        // 算得跳量 	
        jump[0] -= gu_h_x[idx_n]; jump[1] -= gu_h_y[idx_n]; jump[2] -= gu_h_z[idx_n];	
        // 计算误差估计子，长度已经包含在nu里 
        eta[curr_volu] += 0.5*h*h*h*pow((jump[0]*nu[0]+jump[1]*nu[1]+jump[2]*nu[2]), 2);
       }
       else
        eta[curr_volu] +=0.5*h*h*h*pow((jump[0]*nu[0]+jump[1]*nu[1]+jump[2]*nu[2]), 2);
      }//面的循环结束
      Sum_eta += eta[curr_volu];
    }//单元循环结束
    free(gu_h_x); 
    free(gu_h_y); 
    free(gu_h_z);
    total_error[0] = sqrt(Sum_eta);
}

//计算误差估计子for -\laplace u +u^3 = f;
void ComputeErrorIndicatoru3(Fespace3D *fespace,  double *fefunction, double *eta, double *total_error)
{
    Fefunction3D *fefunct;
    fefunct = BuildFefunction3D(fespace, fefunction);
    MESH *mesh = fespace->Mesh;
    ...
    for(curr_volu=0;curr_volu<mesh->Num_Volus_Global;++curr_volu)
    {
      ...
      for(i=0;i<4;i++)
      {
       ...
       GetFefunctValues(fefunct,elem3D,Quad_x,Quad_y,Quad_z,
		       Quad_xi,Quad_eta,Quad_zeta,4,MultiIndex2,Values_FE2);
       sum_tmp = 0.0;
       tmp = -Values_FE2[1]-Values_FE2[2]-Values_FE2[3]+pow(Values_FE2[0], 3)-3*PI*PI*sin(PI*Quad_x)*sin(PI*Quad_y)*sin(PI*Quad_z)*(1/(sqrt(Quad_x*Quad_x+Quad_y*Quad_y+Quad_z*Quad_z)))
                         -2*PI*Quad_x*cos(PI*Quad_x)*sin(PI*Quad_y)*sin(PI*Quad_z)*pow(1/(sqrt(Quad_x*Quad_x+Quad_y*Quad_y+Quad_z*Quad_z)), 3)
                         -2*PI*Quad_y*sin(PI*Quad_x)*cos(PI*Quad_y)*sin(PI*Quad_z)*pow(1/(sqrt(Quad_x*Quad_x+Quad_y*Quad_y+Quad_z*Quad_z)), 3)
                         -2*PI*Quad_z*sin(PI*Quad_x)*sin(PI*Quad_y)*cos(PI*Quad_z)*pow(1/(sqrt(Quad_x*Quad_x+Quad_y*Quad_y+Quad_z*Quad_z)), 3)
                         -3*sin(PI*Quad_x)*sin(PI*Quad_y)*sin(PI*Quad_z)*pow(1/(sqrt(Quad_x*Quad_x+Quad_y*Quad_y+Quad_z*Quad_z)), 3)
                         +3*(Quad_x*Quad_x+Quad_y*Quad_y+Quad_z*Quad_z)*sin(PI*Quad_x)*sin(PI*Quad_y)*sin(PI*Quad_z)*pow(1/(sqrt(Quad_x*Quad_x+Quad_y*Quad_y+Quad_z*Quad_z)), 5)
	                 -pow( sin(PI*Quad_x)*sin(PI*Quad_y)*sin(PI*Quad_z)*(1/(sqrt(Quad_x*Quad_x+Quad_y*Quad_y+Quad_z*Quad_z))), 3);
       sum_tmp = tmp*tmp;
       Sum_K += sum_tmp*Quad_W;
      }//end for i
      eta[curr_volu] += Sum_K*elem3D->Volu*h*h;
...
//计算误差估计子for BEC;
void ComputeErrorIndicatorBEC(Fespace3D *fespace,  double *eigenfunction,double *eigenvalue,  double *eta, double *total_error)
{
    Fefunction3D *fefunct;
    fefunct = BuildFefunction3D(fespace, eigenfunction);
    MESH *mesh = fespace->Mesh;
    ELEMENT3D *elem3D;
    elem3D = malloc(1*sizeof(ELEMENT3D));
    InitialElem3D(mesh,elem3D); 
    ...
    for(curr_volu=0;curr_volu<mesh->Num_Volus_Global;++curr_volu)
    {
      ...
      for(i=0;i<4;i++)
      {
       ...
       GetFefunctValues(fefunct,elem3D,Quad_x,Quad_y,Quad_z,
		       Quad_xi,Quad_eta,Quad_zeta,4,MultiIndex2,Values_FE2);
       sum_tmp = 0.0;
       tmp = -Values_FE2[1]-Values_FE2[2]-Values_FE2[3]+ (Quad_x*Quad_x+Quad_y*Quad_y+Quad_z*Quad_z)*Values_FE2[0]
	     +100*Values_FE2[0]*Values_FE2[0]*Values_FE2[0]-eigenvalue[0]*Values_FE2[0];
       sum_tmp = tmp*tmp;
       Sum_K += sum_tmp*Quad_W;
      }//end for i
      eta[curr_volu] += Sum_K*elem3D->Volu*h*h;
...
//计算误差估计子 single Eigenvalue problem;
void ComputeErrorIndicatorEigen(Fespace3D *fespace,  double *eigenfunction,double *eigenvalue,  double *eta, double *total_error)
{
    Fefunction3D *fefunct;
    fefunct = BuildFefunction3D(fespace, eigenfunction);
    MESH *mesh = fespace->Mesh;
    ...
    //for(i=0;i<mesh->Num_Volus_Global;++i)
    //   eta[i] = 0.0;
    memset(eta,0,sizeof(double)*mesh->Num_Volus_Global);
    // 记录sol_fef的离散梯度 
    double *gu_h_x = malloc(mesh->Num_Volus_Global*sizeof(double));
    double *gu_h_y = malloc(mesh->Num_Volus_Global*sizeof(double));
    double *gu_h_z = malloc(mesh->Num_Volus_Global*sizeof(double));
    ...
    MultiIndex3D MultiIndex1[3]={D100,D010,D001};
    int N_MultiIndex1 = 3;
    //MultiIndex3D MultiIndex2[7]={D000,D110,D101,D011,D200,D020,D002};
    MultiIndex3D MultiIndex2[4]={D000,D200,D020,D002};
    int N_MultiIndex2 = 4;
    double *Values_FE1,*Values_FE2;
    Values_FE1=malloc(sizeof(double)*3);
    ...
    for(curr_volu=0;curr_volu<mesh->Num_Volus_Global;++curr_volu)
    {
      ...
      for(i=0;i<1;i++)//p1元的导数是常数，故取一个点就行
      {
        ...
        GetElementCoord(elem3D,Quad_xi,Quad_eta, Quad_zeta,&Quad_x,&Quad_y,&Quad_z);
        GetFefunctValues(fefunct,elem3D,Quad_x,Quad_y,Quad_z,
		       Quad_xi,Quad_eta,Quad_zeta,N_MultiIndex1,MultiIndex1,Values_FE1);
      }
      gu_h_x[curr_volu]=Values_FE1[0];
      gu_h_y[curr_volu]=Values_FE1[1];
      gu_h_z[curr_volu]=Values_FE1[2];
    ...
    for(curr_volu=0;curr_volu<mesh->Num_Volus_Global;++curr_volu)
    {
      ...
      for(i=0;i<4;i++)
      {
       ...
       GetElementCoord(elem3D,Quad_xi,Quad_eta, Quad_zeta,&Quad_x,&Quad_y,&Quad_z);
       GetFefunctValues(fefunct,elem3D,Quad_x,Quad_y,Quad_z,
		       Quad_xi,Quad_eta,Quad_zeta,N_MultiIndex2,MultiIndex2,Values_FE2);
       sum_tmp = 0.0;
       /*
       tmp = -(1+(Quad_x-0.5)*(Quad_x-0.5))*Values_FE2[4]-((Quad_x-0.5)*(Quad_y-0.5))*Values_FE2[1]-((Quad_x-0.5)*(Quad_z-0.5))*Values_FE2[2]
             -(1+(Quad_y-0.5)*(Quad_y-0.5))*Values_FE2[5]-((Quad_x-0.5)*(Quad_y-0.5))*Values_FE2[1]-((Quad_y-0.5)*(Quad_z-0.5))*Values_FE2[3]
             -(1+(Quad_z-0.5)*(Quad_z-0.5))*Values_FE2[6]-((Quad_x-0.5)*(Quad_z-0.5))*Values_FE2[2]-((Quad_y-0.5)*(Quad_z-0.5))*Values_FE2[3]
	     +(exp((Quad_x-0.5)*(Quad_y-0.5)*(Quad_z-0.5))-eigenvalue[0]*(1+(Quad_x-0.5)*(Quad_y-0.5)*(Quad_z-0.5)))*Values_FE2[0];
       */
       //tmp = -Values_FE2[1]-Values_FE2[2]-Values_FE2[3]-eigenvalue[0]*Values_FE2[0];

       //tmp = -0.5*Values_FE2[1]-0.5*Values_FE2[2]-0.5*Values_FE2[3]-Values_FE2[0]/sqrt(Quad_x*Quad_x+Quad_y*Quad_y+Quad_z*Quad_z)+5.0*Values_FE2[0]-eigenvalue[0]*Values_FE2[0];
       tmp = -0.5*Values_FE2[1]-0.5*Values_FE2[2]-0.5*Values_FE2[3]
             -4.0*Values_FE2[0]/sqrt((Quad_x-0.0001)*(Quad_x-0.0001)+(Quad_y-0.0001)*(Quad_y-0.0001)+(Quad_z-0.0001)*(Quad_z-0.0001))
             -1.0*Values_FE2[0]/sqrt((Quad_x-1.0001)*(Quad_x-1.0001)+(Quad_y-1.0001)*(Quad_y-1.0001)+(Quad_z-1.0001)*(Quad_z-1.0001))
             -1.0*Values_FE2[0]/sqrt((Quad_x+1.0001)*(Quad_x+1.0001)+(Quad_y+1.0001)*(Quad_y+1.0001)+(Quad_z-1.0001)*(Quad_z-1.0001))
             -1.0*Values_FE2[0]/sqrt((Quad_x-1.0001)*(Quad_x-1.0001)+(Quad_y+1.0001)*(Quad_y+1.0001)+(Quad_z+1.0001)*(Quad_z+1.0001))
             -1.0*Values_FE2[0]/sqrt((Quad_x+1.0001)*(Quad_x+1.0001)+(Quad_y-1.0001)*(Quad_y-1.0001)+(Quad_z+1.0001)*(Quad_z+1.0001))-eigenvalue[0]*Values_FE2[0];
       /*
       tmp = -0.5*Values_FE2[1]-0.5*Values_FE2[2]-0.5*Values_FE2[3]-6.0*Values_FE2[0]/sqrt((Quad_x+1.2)*(Quad_x+1.2)+Quad_y*Quad_y+Quad_z*Quad_z)
             -6.0*Values_FE2[0]/sqrt((Quad_x-1.2)*(Quad_x-1.2)+Quad_y*Quad_y+Quad_z*Quad_z) 
             -1.0*Values_FE2[0]/sqrt((Quad_x-3.32)*(Quad_x-3.32)+Quad_y*Quad_y+Quad_z*Quad_z)
             -1.0*Values_FE2[0]/sqrt((Quad_x+3.32)*(Quad_x+3.32)+Quad_y*Quad_y+Quad_z*Quad_z)
             +0*Values_FE2[0];
        */
       sum_tmp = fabs(tmp)*fabs(tmp);
      ...
      for (i = 0; i <4; ++i)
      {
       ...
       { // 若不是边界边 
        ...
        jump[0] -= gu_h_x[idx_n]; jump[1] -= gu_h_y[idx_n]; jump[2] -= gu_h_z[idx_n];	
        // 计算误差估计子，长度已经包含在nu里 
        eta[curr_volu] +=0.0;// 0.125*h*h*h*pow((jump[0]*nu[0]+jump[1]*nu[1]+jump[2]*nu[2]), 2);
       }
       else
        eta[curr_volu] += 0.0;//0.125*h*h*h*pow((jump[0]*nu[0]+jump[1]*nu[1]+jump[2]*nu[2]), 2);
      }//面的循环结束
...
//计算误差估计子for Multiple Eigenvalue;
void ComputeErrorIndicatorMulEigen(Fespace3D *fespace, int nev,  double **eigenfunction,double *eigenvalue,  double *eta, double *total_error)
{

  MESH *mesh = fespace->Mesh;
  double *tmp_totalerror;
  double *tmp_eta;
  tmp_totalerror = malloc(sizeof(double));
  tmp_eta  = malloc(sizeof(double)*mesh->Num_Volus_Global);
  int i,j;
  for(i=0;i<mesh->Num_Volus_Global;++i)
   eta[i] = 0.0;
  total_error[0] = 0.0;
  printf("============================\n");
  double tmp;
  Fefunction3D *fefunc1;
  for(i=0;i<nev;++i)
  {
    fefunc1 = BuildFefunction3D(fespace,eigenfunction[i]);
    NormalizationUserfunc(fefunc1);
  }
  /*
  for(i=0;i<nev;++i)
  {
    tmp = fabs(eigenfunction[i][0]);
    for(j=0;j<fespace->DOF_ALL;++j)
    {
       if(fabs(eigenfunction[i][j])>fabs(tmp))
          tmp = fabs(eigenfunction[i][j]);
    }
    for(j=0;j<fespace->DOF_ALL;++j)
       eigenfunction[i][j]/= tmp;
  }
  */

  for(i=0;i<nev;++i) 
  {
    ComputeErrorIndicatorEigen(fespace,eigenfunction[i],eigenvalue+i, tmp_eta,tmp_totalerror);
    printf("%f ",tmp_totalerror[0]);
    for(j=0;j<mesh->Num_Volus_Global;++j)
      eta[j] += tmp_eta[j];
    
    total_error[0] += tmp_totalerror[0];
  }
  printf("\n");
  free(tmp_eta);
  free(tmp_totalerror);
